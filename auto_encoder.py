...
class AutoEncoder:
    def __init__(self, input_set, weights_path, RECEIVED_PARAMS, encoding_dim=3):
    ...
    def _encoder(self):
        ...
        encoded1 = Dense(self.RECEIVED_PARAMS['layer_1'], activation=self.RECEIVED_PARAMS['activation'])(inputs)
        dropout1 = Dropout(self.RECEIVED_PARAMS['dropout'])(encoded1)
        encoded2 = Dense(self.RECEIVED_PARAMS['layer_2'], activation=self.RECEIVED_PARAMS['activation'])(dropout1)
        dropout2 = Dropout(self.RECEIVED_PARAMS['dropout'])(encoded2)
        encoded3 = Dense(self.encoding_dim, activation=self.RECEIVED_PARAMS['activation'])(dropout2)
    ...
    def _decoder(self):
        ...
        decoded1 = Dense(self.RECEIVED_PARAMS['layer_2'], activation=self.RECEIVED_PARAMS['activation'])(inputs)
        dropout1 = Dropout(self.RECEIVED_PARAMS['dropout'])(decoded1)
        decoded2 = Dense(self.RECEIVED_PARAMS['layer_1'], activation=self.RECEIVED_PARAMS['activation'])(dropout1)
        dropout2 = Dropout(self.RECEIVED_PARAMS['dropout'])(decoded2)
        decoded3 = Dense(self.input_shape, activation=self.RECEIVED_PARAMS['activation'])(dropout2)
    ...
    def fit_autoencoder(self, train_x, batch_size=10, epochs=300):
        ...
        if self.RECEIVED_PARAMS['optimizer'] == 'adam':
            opt = tensorflow.keras.optimizers.Adam(lr=float(self.RECEIVED_PARAMS['learning_rate']), beta_1=0.9,
                                                   beta_2=0.999, epsilon=1e-8)
        if self.RECEIVED_PARAMS['optimizer'] == 'SGD':
            opt = tensorflow.keras.optimizers.SGD(learning_rate=float(self.RECEIVED_PARAMS['learning_rate']))
        self.autoencoder_model.compile(optimizer=opt, loss='mse', metrics=['accuracy'])
        log_dir = './log/'
