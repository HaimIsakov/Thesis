...
try:
    ...
    if RUN_AE or RUN_AE_DEFAULT:
        ...
        if RUN_AE:
            ...
            ae = AutoEncoder(np.array(train_X), root, RECEIVED_PARAMS, encoding_dim=ENCODING_DIM)
            ae_model = ae.encoder_decoder()
            train_results = ae.fit_autoencoder(np.array(train_X), batch_size=RECEIVED_PARAMS['batch_size'], epochs=EPOCHS)
            ae.save_ae()
            ...
            # accuracy
            acc_0_mis = calc_accuracy_zero_padding(test_X, y, encoder_dir)

            # # report intermediate result
            # nni.report_intermediate_result(acc_2_mis)

            # report final result
            LOG.debug('Final result is: %d', acc_0_mis)
            nni.report_final_result(acc_0_mis)

            # Calculate the test MSE

            predict_test = ae_model.predict(test_X)
            predict_train = ae_model.predict(train_X)

            print(f'\nThe error of the train is: {mean_squared_error(train_X, predict_train)}')
            print(f'\nThe error of the test is: {mean_squared_error(test_X, predict_test)}')
    ...
    if RUN_V_AE and V_S != 'None':
        ...
        # train  auto-encoder model
        v_ae = VAutoEncoder(np.array(train_X), root, V_ONE_HOT_LEN, RECEIVED_PARAMS, encoding_dim=ENCODING_DIM)
        ...
        # accuracy
        acc_0_mis = calc_accuracy_zero_padding(test_X, y, v_encoder_dir)
    ...
    if RUN_TSNE:
        ...
        for j, tag in enumerate(classes):
            ...
            # scatter_tsne(T, inds, tag, 'Other', os.path.join(path, 'tsne_results_' + tag), color)
    # KDE ----------------------------------------------
